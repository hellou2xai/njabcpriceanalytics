...
| `one_cs_case_price(rec)` | NEW | The buyer-facing **1-case price**: `frontline_case_price` minus the single largest full-month CPL discount that already qualifies at **1 case** (`best_disc_at(_disc_tiers_min(rec), 1, pack)`). This is the "price after 1 CS discount" every comparison/card surface shows, NOT raw `frontline_case_price`. Precomputed into `sku_offer.one_cs_case_price`; consumers read that column, never re-derive it in a router. `/catalog/search` + `semantic-search` rows are read straight from `cpl_enriched` (which lacks the `sku_offer` column), so they stamp the same column post-query via this helper (`_attach_one_cs`) — the canonical function, not hand-rolled math — so the Products grid header reads the 1-case price like every other surface. |
