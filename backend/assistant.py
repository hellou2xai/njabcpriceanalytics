...
_DATA_TOOLS = {
    ...
    "edition_compare": (_t_edition_compare, "Compare ONE distributor across two CPL editions (price-file periods), defaulting to the latest two, and surface what changed in effective NET cost terms (not raw frontline). Returns a summary (counts that rose/fell/added/removed/RIP-changed) and per-product the net-cost change (case + bottle, $ + %) PLUS which underlying layer moved (frontline / discount / RIP gained/lost/modified), with added vs removed labelled. Args: distributor (required), older + newer (editions like '2026-05'; omit for latest two), match (a product, switches to single-product scope — MUST include the FULL qualifier from the user's query: 'glenlivet jamaican' not 'glenlivet'; 'jack honey' not 'jack daniels'), change (increase|decrease|added|removed|rip), limit. Use for 'what changed at Fedway from May to June', 'biggest price increases this edition at Allied', 'which products did Opici drop', 'how did X's price change month over month at Fedway'."),
...
_SYSTEM = (
    ...
    "PRODUCT SCOPE — KEEP THE FULL QUALIFIER: when the user names a specific product variant "
    "(e.g. 'glenlivet jamaican', 'jack honey', 'absolut lime', 'casamigos blanco'), pass the FULL "
    "qualifier as the match= param — never strip it to just the brand ('glenlivet', 'jack', "
    "'absolut'). Dropping the variant makes the tool return the entire brand family (50+ products) "
    "when the user wanted ONE item. Equally, show_on_screen and top_products must use the full "
    "qualifier as q= so the grid shows only the named variant, not every product from that brand. "
