...
export interface RipProgramTier { qty: number; unit: 'case' | 'btl'; amt: number }
...
export function ripPrograms(tiers?: CatalogTier[] | null): RipProgram[] {
  ...
  for (const t of tiers ?? []) {
    ...
    if (!g) { g = { code, description: t.description ?? null, tiers: [] }; out.push(g); }
    ...
    else g.tiers.push({ qty: t.qty, unit: u, amt: t.amount });
  }
...
/** "2cs $60 · 5cs $75" — the ladder in the portal's own shorthand. */
export function programSummary(p: RipProgram): string {
  return p.tiers.map(t => `${t.qty}${t.unit === 'case' ? 'cs' : 'btl'} ${fmtAmt(t.amt)}`).join(' · ');
}

/** Rebate the program pays at `qty` of its unit (0 when below the entry tier). */
export function programPayAt(p: RipProgram, qty: number): number {
  let best = 0;
  for (const t of p.tiers) if (t.qty <= qty) best = Math.max(best, t.amt);
  return best;
}

/** A different program that pays MORE than the current one at a comparable
 *  commitment (the candidate's entry quantity, or the current quantity once
 *  past it). Returns the suggestion or null. */
export function betterProgram(
  ...
  for (const p of programs) {
    ...
    const atQty = Math.max(p.tiers[0].qty, haveQty);
