...
function Stepper({ label, value, onChange }: { label: string; value: number; onChange: (n: number) => void }) {
  // The input keeps a LOCAL draft while the user types, committing only on blur /
  // Enter — so a multi-digit entry like "15" isn't sent (and clamped back) on
  // every keystroke. The +/- buttons step and commit immediately.
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
      ...
      <input type="text" inputMode="numeric" value={shown} placeholder="0"
